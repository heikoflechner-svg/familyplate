...
export async function POST(req: NextRequest) {
  ...
  const allergenMapping = 'Wichtige Allergen-Quellen zur Erkennung: Gluten=Weizenmehl/Pasta/Brot/Pizzateig/Paniermehl/normales Mehl. Laktose=Milch/Käse/Butter/Sahne/Joghurt/Quark/Schmand/Frischkäse. Kasein=Milch/Käse/Butter/Sahne/Joghurt/Quark/Schmand/Frischkäse/alle Milchprodukte. Nüsse=Mandeln/Walnüsse/Haselnüsse/Cashews/Erdnüsse/Nussöle/Nussmus.'
  const prompt = `Du bist Rémy. Plane ${slotHinweis} für ${planTage.join(', ')} für Familie Flechner. Profil: ${familienProfil}.${wishHinweis}${historyHinweis}${behalteneHinweis} Gefriertruhe: ${freezerList}. Speisekammer: ${pantryList}. Nutze Gefriertruhe/Speisekammer wenn sinnvoll – Artikel mit [DRINGEND] müssen diese Woche eingeplant werden. Weise pro Tag+Slot Küchenchef zu (MA PA TI) nach Fairness. WICHTIG: Wähle immer normale, typische Familiengerichte – nie vorsorglich angepasste Varianten. Ändere Gerichtsnamen nie. Allergen-Pflicht: Prüfe für JEDES Gericht ALLE Zutaten gegen ALLE Unverträglichkeiten im Profil. ${allergenMapping} Wenn eine Zutat eines gewählten Gerichts eine Unverträglichkeit verletzt, trag die Ersatz-Zutat für die betroffene Person in ersetzteZutaten ein als "Menge Zutat (für Person)", z.B. "1 Packung glutenfreier Pizzateig (für Heiko)" oder "laktosefreie Milch statt Milch (für Heiko)". Prüfe auch versteckte Quellen: Sahnesaucen→Kasein/Laktose, Bechamel→Mehl+Butter, paniert→Gluten. Wenn wirklich keine Unverträglichkeit betroffen ist, setze ersetzteZutaten auf []. Rezepte: 5–8 Zutaten, 4–6 knappe Schritte mit Mengenangaben. Antworte NUR als reines JSON ohne Markdown-Codeblock: {"woche":[${beispiele.join(',')}],"rezepte":{"GerichtName":${rezeptBeispiel}}} — Für jedes Gericht in woche muss ein Eintrag in rezepte stehen. typ: frisch, tiefkühl, speisekammer oder gefriertruhe.`
